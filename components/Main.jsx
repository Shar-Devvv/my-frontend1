now in this name is editbale now  remove input button and make the lvie preview alwasy showing this normal any name and whenw e upllaod image and click save and rpevieable direcgly edits name save s and remove input field make intrafce like and page of resume is open adn where people can edit what they need for now only name do this  code is editbale just some save reiviw functionaly and input field nremove "use client" 
...
const ResumePreview = ({ name, uploadedImage, resumeContent, handleContentChange }) => {
  const { summary } = resumeContent;
    ...
    >
      ...
      <section style={{ marginBottom: '25px' }}>
        ...
        <p
          contentEditable
          suppressContentEditableWarning
          onInput={(e) => handleContentChange('summary', e.target.textContent)}
          style={{ fontSize: '0.95rem', lineHeight: '1.6', color: '#374151' }}
        >
          {summary}
        </p>
      </section>
...
  const [selectedResume, setSelectedResume] = useState(null); // New state for viewing/editing
 const [resumeContent, setResumeContent] = useState({
    summary: "Highly motivated and results-oriented professional with 5+ years of experience in MERN stack development, specializing in scalable web applications and intuitive user interfaces. Seeking a challenging role to leverage expertise in Node.js, React, and MongoDB.",
    title:"Frontend Devleoper / UI Specialist",
    name:"Your Name"
    // You can add more sections here: experience, education, etc.
  });
  // State to track which section is currently being edited
  const [editingSection, setEditingSection] = useState(null); // e.g., 'summary' or null
...
  return (
    <div className='bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500' style={{ fontFamily: 'Inter, sans-serif', backgroundColor: '#f9fafb', minHeight: '100vh', padding: '20px' }}>
...
<motion.div initial={{ y: -100, opacity: 0 }} 
    ...
    whileHover={{ 
     ...
     className=" top flex items-center space-x-16 p-1 min-w-fit bg-white/10  rounded-xl px-6 py-3 shadow-lg border border-white/20 bottom-[23.5%] text-white  justify-center relative" style={{opacity: 1,perspective: 1000 }}  >
        ...
        <Link href={"/"}><button className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md transition duration-200 hover:scale-[1.03]" >Home</button></Link>
...
      {/* Main Content Area */}
      <motion.div  className='main  shadow-lg border ' style={{ 
          display: 'flex',
...
        <motion.div initial={{x: -100}} animate={{x: 0}} transition={{duration: 0.2}} style={{ width: '428px', padding: '20px', borderRight: '1px solid #e5e7eb', minHeight: '80vh' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#1f2937', marginBottom: '15px' }}>Live Resume Preview</h2>
          {uploadedImage || name ? (
                <ResumePreview name={name} uploadedImage={uploadedImage} resumeContent={resumeContent} handleContentChange={handleContentChange}/>
            ) : (
...
        <motion.div initial={{x: 200}} animate={{x: 0}} transition={{duration: 0.2}} style={{ flexGrow: 1, padding: '30px 40px' }}>
...
<button
  ...
  className="h-[45px]  bg-indigo-600 text-white p-3  relative right-[10%] rounded-md"
...
                <motion.h1 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '8px' }}>1. Enter your Username</motion.h1>
                <motion.input initial={{ opacity: 0 }} animate={{ opacity: 1,focusable: true }} whileHover={{ scale: 1.03 }}
                    type="text"
                    value={name}
...
            <motion.button animate={{ scale: 1.1 }} whileHover={{ scale: 1.2 }}
                onClick={handleClick}
                style={{
...
            </motion.button>
            
            <p style={{ marginTop: '15px', position:'relative', right: '10px', color: 'white', fontSize: '0.85rem' }}>*This uploads the image and updates the Recents list if your backend saves new files automatically.</p>


            {/* Download Button Section */}
            {uploadedImage && (
                <div style={{ marginTop: '40px', padding: '20px', border: '1px solid #e0e7ff', width:'410px', borderRadius: '8px', backgroundColor: 'bg-white/10' }}>
                    <h2  style={{ fontSize: '1.5rem', fontWeight: '600', color: '#1f2937', textalign: 'center', marginBottom: '15px' }}>Download Ready</h2>
                    <button 
